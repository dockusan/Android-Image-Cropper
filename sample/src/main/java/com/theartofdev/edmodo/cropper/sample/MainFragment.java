    ...
    private void handleCropResult(CropImageView.CropResult result) {
        if (result.getError() == null) {
            ...
            if (result.getUri() != null) {
                ...
                RectF rectF = new RectF(0, 0, result.getBitmap().getWidth(), result.getBitmap().getHeight());
                switch (mCropImageView.getCropShape()) {
                    case RECTANGLE:
                        CropResultActivity.mImage = result.getBitmap();
                        break;
                    case OVAL:
                        CropResultActivity.mImage = CropImage.toShapeBitmap(result.getBitmap(), PathUtils.generateOvalPath(rectF, new Path()));
                        break;
                    case TRIANGLE:
                        CropResultActivity.mImage = CropImage.toShapeBitmap(result.getBitmap(), PathUtils.generateTrianglePath(rectF, new Path()));
                        break;
                    case HEART:
                        CropResultActivity.mImage = CropImage.toShapeBitmap(result.getBitmap(), PathUtils.generateHeartPath(rectF, new Path()));
                        break;
                    case STAR:
                        CropResultActivity.mImage = CropImage.toShapeBitmap(result.getBitmap(), PathUtils.generateStarPath(rectF, new Path()));
                        break;
                }
            }
